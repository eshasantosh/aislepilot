...
export default function MapPage() {
  const [displayListForMap, setDisplayListForMap] =
    useState<CategorizeItemsOutput | null>(null);
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});
  const [itemQuantities, setItemQuantities] = useState<Record<string, number>>(
    {},
  );
  const [userAddedSuggestions, setUserAddedSuggestions] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    try {
      const savedCategorizedList = localStorage.getItem(
        LOCAL_STORAGE_KEYS.CATEGORIZED_LIST,
      );
      if (savedCategorizedList)
        categorizedListFromAI = JSON.parse(savedCategorizedList);

      const savedCheckedItems = localStorage.getItem(
        LOCAL_STORAGE_KEYS.CHECKED_ITEMS,
      );
      if (savedCheckedItems) checksFromStorage = JSON.parse(savedCheckedItems);

      const savedItemQuantities = localStorage.getItem(
        LOCAL_STORAGE_KEYS.ITEM_QUANTITIES,
      );
      if (savedItemQuantities)
        quantitiesFromStorage = JSON.parse(savedItemQuantities);

      const savedUserAddedSuggestions = localStorage.getItem(
        LOCAL_STORAGE_KEYS.USER_ADDED_SUGGESTIONS,
      );
      if (savedUserAddedSuggestions)
        addedSuggestionsFromStorage = JSON.parse(savedUserAddedSuggestions);
    } catch (storageAccessError) {
      console.error(
        "Error accessing localStorage on map page:",
        storageAccessError,
      );
      toast({
        variant: "destructive",
    ...
    if (categorizedListFromAI && categorizedListFromAI.categorizedAisles) {
      const processedListForMap: CategorizeItemsOutput = {
        categorizedAisles: [],
      };
      ...
      if (uniquePoints.length > 0) {
        const { pathCoords, visitOrder, pathSegmentsCoords } =
          findOptimalPath(uniquePoints);
        ...
        const shoppingOrder = visitOrder
          .map((pointName) => points[pointName].aisle)
          .filter(Boolean);
        const sortedAisles = [...processedListForMap.categorizedAisles].sort(
          (a, b) => {
  ...
  useEffect(() => {
    if (!orderedAisles.length || !displayListForMap?.categorizedAisles.length)
      return;
    ...
    for (let i = 0; i < pointsToVisit.length; i++) {
      ...
      const aislesForThisPoint = displayListForMap.categorizedAisles.filter(
        (aisle) =>
          aisleToPointName[aisle.aisleName.toLowerCase()] === pointName,
      );
  ...
  useEffect(() => {
    if (!isLoading) {
      try {
        ...
        console.error("Error saving item quantities:", e);
      }
    }
  ...
  const requestCameraPermission = async () => {
    if (
      ...
      toast({
        ...
        description: "Browser does not support camera.",
      });
    }
  };
  ...
  const handleConfirmScannedItem = () => {
    ...
    setDisplayListForMap((prevList) => {
      ...
      const aisleIndex = prevList.categorizedAisles.findIndex(
        (a) => a.aisleName.toLowerCase() === aisleName.toLowerCase(),
      );
      ...
      if (aisleIndex > -1) {
        ...
        const itemExists = targetAisle.items.some(
  ...
  const handleItemInteractionOnMap = useCallback(
    (itemName: string, _aisleName: string, _isInitialSuggestion: boolean) => {
  ...
  return (
    <main className="relative flex-grow flex flex-col overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <Dialog onOpenChange={handleCheckoutOpen}>
                  <DialogTrigger asChild>
                    <Button
                      size="sm"
                      className="shadow-sm hover:shadow-md transition-shadow bg-accent text-accent-foreground hover:bg-accent/90"
                    >
                  ...
                  <DialogContent className="sm:max-w-md bg-card border-border shadow-xl">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Scan this barcode at any self-checkout counter to
                      ...
                      <ul className="space-y-3 max-h-60 overflow-y-auto pr-2">
                        {completedItemsInCart.map((itemName) => {
                          ...
                          return (
                            ...
                            >
                              <span className="font-medium flex-grow mb-2 sm:mb-0">
                                {itemName}
                              </span>
                              <div className="flex items-center justify-between sm:justify-end gap-2">
                                <div className="flex items-center gap-2">
                                  <Button
                                    ...
                                    className="h-7 w-7"
                                    onClick={() =>
                                      handleDecreaseQuantity(itemName)
                                    }
                                    disabled={quantity <= 1}
                                  ...
                                  <Button
                                    ...
                                    className="h-7 w-7"
                                    onClick={() =>
                                      handleIncreaseQuantity(itemName)
                                    }
      ...
      {scannedItem && (
        <AlertDialog
          open={!!scannedItem}
          onOpenChange={() => setScannedItem(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                Add "{scannedItem.name}" to Cart?
              </AlertDialogTitle>
              <AlertDialogDescription>
                This item was detected by the scanner. Adjust the quantity and
                add it to your cart.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex items-center justify-center gap-4 py-4">
              <Button
                variant="outline"
                size="icon"
                onClick={() => handleScannedQuantityChange(-1)}
                disabled={scannedItem.quantity <= 1}
                aria-label={`Decrease ${scannedItem.name}`}
              >
                {" "}
                <Minus className="h-4 w-4" />{" "}
              </Button>
              ...
              <Button
                ...
                size="icon"
                onClick={() => handleScannedQuantityChange(1)}
                aria-label={`Increase ${scannedItem.name}`}
              >
                {" "}
                <Plus className="h-4 w-4" />{" "}
              </Button>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setScannedItem(null)}>
                Cancel
              </AlertDialogCancel>
              <AlertDialogAction onClick={handleConfirmScannedItem}>
                Add to Cart
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
